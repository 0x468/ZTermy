...
[[nodiscard]] std::expected<ApplicationSettings, ApplicationSettingsStoreError> parseSettings(const QJsonObject &root)
{
    ...
    const auto theme = ztermy::config::parseThemePreference(themeValue.toString());
    const auto backdrop = ztermy::config::parseBackdropPreference(backdropValue.toString());
    const auto effectsTier = version >= effectsTierSchemaVersion
                                 ? ztermy::config::parseEffectsTier(effectsTierValue.toString())
                                 : std::optional{EffectsTier::full};
    const auto accent = version >= accentSchemaVersion ? ztermy::config::parseAccentPreference(accentValue.toString())
                                                       : std::optional{AccentPreference::ztermy};
    const auto cursor = ztermy::config::parseCursorPreference(cursorValue.toString());
    const auto terminalRightClick =
        version >= terminalInteractionSchemaVersion
            ? ztermy::config::parseTerminalRightClickPreference(terminalRightClickValue.toString())
    ...
    const auto terminalMiddleClick =
        version >= terminalSelectionSettingsSchemaVersion
            ? ztermy::config::parseTerminalMiddleClickPreference(terminalMiddleClickValue.toString())
            : std::optional{TerminalMiddleClickPreference::disabled};
    const auto localShell = version >= localShellSchemaVersion
                                ? ztermy::config::parseLocalShellPreference(localShellValue.toString())
                                : std::optional{LocalShellPreference::automatic};
    const auto credentialStorage =
        version >= credentialStorageSchemaVersion
            ? ztermy::config::parseCredentialStoragePreference(credentialStorageValue.toString())
            : std::optional{CredentialStoragePreference::automatic};
    const auto language = version >= localizationSchemaVersion
                              ? ztermy::config::parseLanguagePreference(languageValue.toString())
                              : std::optional{LanguagePreference::system};
    const auto aiProvider = version >= aiProviderSchemaVersion
                                ? ztermy::config::parseAiProviderPreference(aiProviderValue.toString())
    ...
    const auto aiPermission =
        version >= aiPermissionSchemaVersion
            ? (version < aiPermissionContractSchemaVersion && aiPermissionValue.toString() == QStringLiteral("edit")
                   ...
                   : ztermy::config::parseAiPermissionPreference(aiPermissionValue.toString()))
            : std::optional{AiPermissionPreference::ask};
    const auto aiReasoning = version >= aiReasoningSchemaVersion
                                 ? ztermy::config::parseAiReasoningPreference(aiReasoningValue.toString())
                                 : std::optional{AiReasoningPreference::automatic};
    const auto aiProxy = version >= aiProxySchemaVersion
                             ? ztermy::config::parseAiProxyPreference(aiProxyValue.toString())
