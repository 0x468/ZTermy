...
private slots:
    ...
    void migratesSchema31AndPreservesProvider();
    void migratesSchema32AndPersistsConnectionHistoryPreference();
    void migratesSchema33AndPersistsEffectsTier();
    ...
    void migratesMaterialSchemaWithUnifiedTerminalBackground();
    ...
    void migratesPrePerformanceSchemaWithPerformanceDisabled();
    void migratesPreInteractionSchemaWithContextMenuDefault();
    void migratesPreSelectionInteractionSchemaWithMainstreamDefaults();
    void migratesPreSelectionRetentionSchemaWithDismissDefault();
    void migratesPromptWordDelimitersWithoutOverwritingCustomValue();
    void migratesPreLocalShellSchemaWithAutomaticDefault();
...
void ApplicationSettingsTests::migratesSchema35AndPersistsThemeSlots()
{
    ...
    original.terminalTheme = QStringLiteral("nord");
    original.aiModel = QStringLiteral("preserved-model");
    original.theme = ztermy::config::ThemePreference::system;
    QVERIFY(store.save(original));
    QFile file(path);
    QVERIFY(file.open(QIODevice::ReadOnly));
    auto json = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    json.insert(QStringLiteral("version"), 35);
    json.remove(QStringLiteral("lightTheme"));
    json.remove(QStringLiteral("darkTheme"));
    QVERIFY(writeFile(path, QJsonDocument(json).toJson()));
    auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(*loaded, original);
    loaded->lightTheme = QStringLiteral("solarized-light");
    loaded->darkTheme = QStringLiteral("catppuccin-mocha");
    loaded->accent = ztermy::config::AccentPreference::theme;
...
void ApplicationSettingsTests::migratesSchema36AndUnifiesWindowOpacity()
{
    ...
    const ztermy::config::ApplicationSettingsStore store(path);
    ztermy::config::ApplicationSettings original;
    original.backdropOpacity = 0.7;
    original.terminalBackgroundOpacity = 0.4;
    original.aiModel = QStringLiteral("preserved-model");
    QVERIFY(store.save(original));
    ...
    auto json = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    json.insert(QStringLiteral("version"), 36);
    QVERIFY(writeFile(path, QJsonDocument(json).toJson()));

    auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(loaded->backdropOpacity, 0.7);
    QCOMPARE(loaded->terminalBackgroundOpacity, 0.0);
    QCOMPARE(loaded->aiModel, QStringLiteral("preserved-model"));
    loaded->backdrop = ztermy::config::BackdropPreference::aero;
    QVERIFY(store.save(*loaded));
    QCOMPARE(store.load(), loaded);
}
...
void ApplicationSettingsTests::migratesSchema31AndPreservesProvider()
{
    ...
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    ztermy::config::ApplicationSettings original;
    original.aiBaseUrl = QStringLiteral("https://example.test/v1");
    original.aiModel = QStringLiteral("kept-model");
    original.localShell = ztermy::config::LocalShellPreference::windowsPowerShell;
    const ztermy::config::ApplicationSettingsStore store(path);
    QVERIFY(store.save(original));
    QFile file(path);
    QVERIFY(file.open(QIODevice::ReadOnly));
    auto json = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    json.insert(QStringLiteral("version"), 31);
    json.remove(QStringLiteral("windowInteraction"));
    QVERIFY(writeFile(path, QJsonDocument(json).toJson()));
    auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(*loaded, original);
    loaded->windowInteraction = {.singleInstance = false,
                                 .navigationWidth = 56,
                                 .navigationExpandedWidth = 250,
                                 .tabDoubleClick = QStringLiteral("close"),
                                 .tabCloseButton = QStringLiteral("hidden")};
    QVERIFY(store.save(*loaded));
    QCOMPARE(store.load(), loaded);
    QVERIFY(file.open(QIODevice::ReadOnly));
...
void ApplicationSettingsTests::migratesSchema32AndPersistsConnectionHistoryPreference()
{
    QTemporaryDir directory;
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    const ztermy::config::ApplicationSettingsStore store(path);
    ztermy::config::ApplicationSettings original;
    original.aiModel = QStringLiteral("kept-model");
    original.aiCredentialReference = QStringLiteral("opaque-reference");
    original.windowInteraction.navigationWidth = 250;
    QVERIFY(store.save(original));
    QFile file(path);
    QVERIFY(file.open(QIODevice::ReadOnly));
    auto json = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    json.insert(QStringLiteral("version"), 32);
    json.remove(QStringLiteral("connectionHistoryEnabled"));
    QVERIFY(writeFile(path, QJsonDocument(json).toJson()));
    auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(*loaded, original);
    loaded->connectionHistoryEnabled = false;
    QVERIFY(store.save(*loaded));
    QCOMPARE(store.load(), loaded);
    QVERIFY(file.open(QIODevice::ReadOnly));
    json = QJsonDocument::fromJson(file.readAll()).object();
    ...
    QVERIFY(writeFile(invalidPath, QJsonDocument(json).toJson()));
    QVERIFY(!ztermy::config::ApplicationSettingsStore(invalidPath).load());
}

void ApplicationSettingsTests::migratesSchema33AndPersistsEffectsTier()
{
    QTemporaryDir directory;
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    const ztermy::config::ApplicationSettingsStore store(path);
    ztermy::config::ApplicationSettings original;
    original.connectionHistoryEnabled = false;
    original.backdrop = ztermy::config::BackdropPreference::mica;
    QVERIFY(store.save(original));
    QFile file(path);
    QVERIFY(file.open(QIODevice::ReadOnly));
    auto json = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    json.insert(QStringLiteral("version"), 33);
    json.remove(QStringLiteral("effectsTier"));
    QVERIFY(writeFile(path, QJsonDocument(json).toJson()));
    auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(*loaded, original);
    QCOMPARE(loaded->effectsTier, ztermy::config::EffectsTier::full);
    loaded->effectsTier = ztermy::config::EffectsTier::reduced;
    QVERIFY(store.save(*loaded));
    QCOMPARE(store.load(), loaded);
    QVERIFY(file.open(QIODevice::ReadOnly));
    json = QJsonDocument::fromJson(file.readAll()).object();
    ...
    QVERIFY(writeFile(invalidPath, QJsonDocument(json).toJson()));
    QVERIFY(!ztermy::config::ApplicationSettingsStore(invalidPath).load());
    QCOMPARE(ztermy::config::effectsTierToken(ztermy::config::EffectsTier::off), QStringLiteral("off"));
    QCOMPARE(ztermy::config::parseEffectsTier(QStringLiteral("off")), ztermy::config::EffectsTier::off);
    QVERIFY(!ztermy::config::parseEffectsTier(QStringLiteral("Full")));
}

void ApplicationSettingsTests::migratesSchema34AndPersistsTerminalTheme()
{
    QTemporaryDir directory;
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    const ztermy::config::ApplicationSettingsStore store(path);
    ztermy::config::ApplicationSettings original;
    original.effectsTier = ztermy::config::EffectsTier::reduced;
    QVERIFY(store.save(original));
    QFile file(path);
    QVERIFY(file.open(QIODevice::ReadOnly));
    auto json = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    json.insert(QStringLiteral("version"), 34);
    json.remove(QStringLiteral("terminalTheme"));
    QVERIFY(writeFile(path, QJsonDocument(json).toJson()));
    auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(*loaded, original);
    QCOMPARE(loaded->terminalTheme, QStringLiteral("ztermy-dark"));
    loaded->terminalTheme = QStringLiteral(" solarized-dark ");
    QVERIFY(store.save(*loaded));
    const auto reloaded = store.load();
    QVERIFY(reloaded);
    QCOMPARE(reloaded->terminalTheme, QStringLiteral("solarized-dark"));
    QVERIFY(file.open(QIODevice::ReadOnly));
    json = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    QCOMPARE(json.value(QStringLiteral("version")).toInt(), 38);
    ...
    QVERIFY(writeFile(invalidPath, QJsonDocument(json).toJson()));
    QVERIFY(!ztermy::config::ApplicationSettingsStore(invalidPath).load());
    json.insert(QStringLiteral("terminalTheme"), QStringLiteral("../escape"));
    QVERIFY(writeFile(invalidPath, QJsonDocument(json).toJson()));
    QVERIFY(!ztermy::config::ApplicationSettingsStore(invalidPath).load());
    json.remove(QStringLiteral("terminalTheme"));
    QVERIFY(writeFile(invalidPath, QJsonDocument(json).toJson()));
...
void ApplicationSettingsTests::savesAndLoadsEveryPreference()
{
    ...
    const ztermy::config::ApplicationSettings expected{
        ...
        .terminalWordDelimiters = QStringLiteral(" |,"),
        ...
        .localShell = ztermy::config::LocalShellPreference::nushell,
        .theme = ztermy::config::ThemePreference::light,
        .backdrop = ztermy::config::BackdropPreference::aero,
...
void ApplicationSettingsTests::migratesMaterialSchemaWithUnifiedTerminalBackground()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("settings.json"));
...
void ApplicationSettingsTests::migratesPrePerformanceSchemaWithPerformanceDisabled()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    ...
    auto expected = ztermy::config::ApplicationSettings{};
    expected.backdrop = ztermy::config::BackdropPreference::micaAlt;
    QVERIFY(store.save(expected));
    ...
    auto root = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    root.insert(QStringLiteral("version"), 23);
    root.remove(QStringLiteral("performanceMode"));
    QVERIFY(writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Compact)));

    const auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(loaded->backdrop, ztermy::config::BackdropPreference::micaAlt);
    QVERIFY(!loaded->performanceMode);
...
void ApplicationSettingsTests::migratesPreInteractionSchemaWithContextMenuDefault()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    ...
    auto root = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    root.insert(QStringLiteral("version"), 24);
    root.remove(QStringLiteral("terminalRightClick"));
    QVERIFY(writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Compact)));

    const auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(loaded->terminalRightClick, ztermy::config::TerminalRightClickPreference::contextMenu);
}

void ApplicationSettingsTests::migratesPreSelectionInteractionSchemaWithMainstreamDefaults()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    ...
    auto root = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    root.insert(QStringLiteral("version"), 25);
    root.remove(QStringLiteral("terminalMiddleClick"));
    root.remove(QStringLiteral("terminalWordDelimiters"));
    root.remove(QStringLiteral("terminalScrollRows"));
    QVERIFY(writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Compact)));

    const auto loaded = store.load();
    QVERIFY(loaded);
    QCOMPARE(loaded->terminalMiddleClick, ztermy::config::TerminalMiddleClickPreference::disabled);
    QCOMPARE(loaded->terminalWordDelimiters, QStringLiteral(" \t'\"│`|;,()[]{}<>$@:#~"));
    QCOMPARE(loaded->terminalScrollRows, 3);
}

void ApplicationSettingsTests::migratesPreSelectionRetentionSchemaWithDismissDefault()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    const ztermy::config::ApplicationSettingsStore store(path);
    auto current = ztermy::config::ApplicationSettings{};
    current.keepSelectionAfterCopy = true;
    QVERIFY(store.save(current));

    QFile file(path);
    QVERIFY(file.open(QIODevice::ReadOnly));
    auto root = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    root.insert(QStringLiteral("version"), 26);
    root.remove(QStringLiteral("keepSelectionAfterCopy"));
    QVERIFY(writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Compact)));

    const auto loaded = store.load();
    QVERIFY(loaded);
    QVERIFY(!loaded->keepSelectionAfterCopy);
...
void ApplicationSettingsTests::migratesPromptWordDelimitersWithoutOverwritingCustomValue()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    ...
    auto root = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    root.insert(QStringLiteral("version"), 27);
    root.insert(QStringLiteral("terminalWordDelimiters"), QStringLiteral(" \t'\"│`|;,()[]{}<>$"));
    QVERIFY(writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Compact)));

    const auto migrated = store.load();
    QVERIFY(migrated);
    QCOMPARE(migrated->terminalWordDelimiters, QStringLiteral(" \t'\"│`|;,()[]{}<>$@:#~"));

    root.insert(QStringLiteral("terminalWordDelimiters"), QStringLiteral(" custom"));
    QVERIFY(writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Compact)));
    const auto customized = store.load();
    QVERIFY(customized);
    QCOMPARE(customized->terminalWordDelimiters, QStringLiteral(" custom"));
}

void ApplicationSettingsTests::migratesPreLocalShellSchemaWithAutomaticDefault()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("settings.json"));
    ...
    auto root = QJsonDocument::fromJson(file.readAll()).object();
    file.close();
    root.insert(QStringLiteral("version"), 28);
    root.remove(QStringLiteral("localShell"));
    QVERIFY(writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Compact)));

    const auto migrated = store.load();
    QVERIFY(migrated);
    QCOMPARE(migrated->localShell, ztermy::config::LocalShellPreference::automatic);
...
void ApplicationSettingsTests::rejectsMalformedUnsupportedAndIncompleteDocuments()
{
    ...
    QVERIFY(writeFile(path, QByteArrayLiteral(R"({"version":38})")));
...
void ApplicationSettingsTests::rejectsOutOfRangeValues()
{
    ...
    auto invalidWordDelimiters = ztermy::config::ApplicationSettings{};
    invalidWordDelimiters.terminalWordDelimiters = QString(129, QLatin1Char('x'));
    const auto wordDelimitersResult = store.save(invalidWordDelimiters);
    QVERIFY(!wordDelimitersResult);
    QCOMPARE(wordDelimitersResult.error(), ztermy::config::ApplicationSettingsStoreError::invalidFormat);

    auto invalidScrollRows = ztermy::config::ApplicationSettings{};
    invalidScrollRows.terminalScrollRows = 0;
    const auto scrollRowsResult = store.save(invalidScrollRows);
    QVERIFY(!scrollRowsResult);
    QCOMPARE(scrollRowsResult.error(), ztermy::config::ApplicationSettingsStoreError::invalidFormat);

    invalidScrollRows.terminalScrollRows = 21;
    const auto excessiveScrollRowsResult = store.save(invalidScrollRows);
    QVERIFY(!excessiveScrollRowsResult);
    QCOMPARE(excessiveScrollRowsResult.error(), ztermy::config::ApplicationSettingsStoreError::invalidFormat);
}
