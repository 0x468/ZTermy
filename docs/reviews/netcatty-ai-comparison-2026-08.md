...
- 加密会话历史现在保存完整的已完成 Agent 展示快照：恢复后仍显示 provider 公开思考、
  原生工具卡、引用、token、耗时/重试、费用和截断状态；历史工具卡为不可执行记录，不会
  恢复权限、待批准动作或终端写所有权。
- 连续工具调用已经按助手轮次合并为可折叠的 `Used N tools` 时间线；活动回合默认展开，
  回答完成后自动折叠。每项可检查有界参数与结果并单独复制，恢复历史后仍为不可执行快照。
- 320px 窄侧栏只保留 Agent、历史、新会话和更多入口；导出等低频会话操作收进更多菜单，
...
- Warp 的公开文档同时强调实时终端缓冲区读取和由用户显式附加 Terminal Block；Termius
  ...
  附加或由当前终端工具按需读取。
- Netcatty 新版还提供“完整工具输出存储 + 有界预览句柄”。ztermy 已在 ADR 0094 中完成
  自身实现：head/tail 继续作为快速预览，独立工件按单命令 2 MiB、单终端 8 MiB 封顶，
  完成工件按最旧优先淘汰；AI 从所属终端观察器实时分页，因此能读取本轮新命令及预览中
  已被截掉的中段。缺口、超限和淘汰均显式返回，且一次只复制最多 16 KiB，不阻塞 QML
  渲染线程，也不形成跨终端输出仓库。
