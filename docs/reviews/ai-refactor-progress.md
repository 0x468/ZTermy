...
## 节点 N37 — 设置兼容与无感模型发现（2026-08-21）

**commits**：

- `7cb92d3`（fix(settings): preserve provider configuration after agent removal）
- 模型发现提交待本节点收尾

**范围**：

1. 修复应用设置 schema 从已写入用户磁盘的 20 错误回退为 19，导致整份设置被当作
   未来版本拒绝的问题；schema 20 作为兼容墓碑永久保留，移除的 `aiAgent` 字段只被
   忽略，不再连带丢失供应商 URL、模型和凭据引用；
2. 为真实 schema-20 文档增加迁移夹具，验证供应商设置完整读取、保存后删除废弃字段，
   并锁定保存版本仍为 20；ADR 0101 固化 schema 只能递增、不能回退或复用；
3. 在启动程序、创建新终端、打开 AI 侧栏和保存供应商配置后，使用当前配置与已保存
   凭据静默刷新模型目录；并发触发合并，后台失败保留旧列表且不显示错误；
4. 后台获取不复用手动刷新按钮的 loading 状态，不造成可见闪烁；成功结果仍响应式更新
   模型下拉框；
5. 完成 OpenAI 账号登录与 AI 独立代理调研。当前 ChatGPT/Codex 订阅登录没有公开的
   第三方通用模型 API 合同，不接入 Hermes 复用的 Codex 私有路径；AI-only 代理可通过
   两个独立 `QNetworkAccessManager` 实现，不影响 SSH，方案记录于
   `docs/research/OPENAI_ACCOUNT_AUTH_AND_AI_PROXY.md`。

**验证**：应用设置测试通过；启动、保存配置、新建终端和打开侧栏的模型自动发现聚焦
测试通过；C++ 全量格式门禁通过。当前工作树另有尚未完成的 N36 响应重试改动，其独立
端到端用例仍失败，因此本节点没有把全量 `app-controller` 套件误报为通过。

## 节点 N35 — 类型化供应商错误恢复（2026-08-21）

**commit**: 本节点提交（见 git 历史）

**范围**：

1. 对照 CtrlOps AI Terminal 的认证、限流、模型与连接故障入口，以及 Warp 对无效或
   过期 API key 的不可重试配置错误定义，确认所有失败共用一个“重试”不足以指导恢复；
2. 将既有网络、认证、限流、配额、无效请求、服务端、取消、协议和上下文超限错误码
   保留到控制器与 QML，使用独立领域策略映射“重试 / AI 设置 / 新建对话”；
3. 最新失败回复卡片直接承载错误、恢复提示和相关操作；有卡片锚点时隐藏顶部全局错误，
   避免同一失败重复两次。清空对话同步清除旧助手消息锚点；
4. 认证失败可一次点击直达“设置 → AI”，修复后仍可重试原请求；上下文超限只建议新建
   对话或缩短上下文，不提供原样重复超限请求的内联重试；
5. 本地 HTTP 401 夹具覆盖生产错误解析、控制器恢复模型和真实 QML 窗口；常规与 260px
   紧凑侧栏均验证按钮边界、可访问名称、无重复顶部错误和设置导航；ADR 0099 固化合同。

**验证**：MSVC 动态 Debug 与静态 Release 完整构建通过，两套全量测试均为
`115/115`；250 个 clang-tidy 任务以 warnings-as-errors 通过，C++ 格式、48 个 QML
文件的 `qmlformat`/`qmllint` 和 `1782/1782` 翻译门禁通过。真实 Qt Quick 布局 smoke
生成常规与紧凑错误恢复截图，合同记录 `regular=1`、`compact=1`、
`settingsOpened=1`、`globalErrorVisible=0`。便携 ZIP 与 MSI 已重新生成，SHA-256 分别为
`270e85eab1ff3584a52be9be22de80bb2ebc17481b6173628683b85aae206c29` 和
`29e0910cf5a38bde1c31f329978c1f57fdc3533024d041496786fe3a6ebae152`；当前主机的
...
1. 对照 Warp Blocks as Context、Wave AI 和本地 NetCatty 产品行为，确认显式上下文不仅
   要在发送前可见，也必须在发送后与提出问题的用户消息保持可追溯关联；
2. 为用户消息增加有界来源摘要，展示标题、类型、证据质量以及截断/脱敏状态；草稿发送
   后清空，但来源 chip 留在原用户消息上并随加密历史恢复；
3. 修正上下文证据此前落在助手消息之后的顺序：现在 Provider 回放为“用户问题 → 隐藏
   证据 → 助手回答”，工具证据仍归属实际执行工具的助手消息；
4. QML 只获取摘要而不获取隐藏证据正文；260px 窄侧栏使用可换行 chip，标题省略和
   tooltip 共同保留可读性；
5. 模型与持久化边界分别限制最多 16 项、标题 1,024 UTF-8 字节和类型/质量 64 字节，
   拒绝畸形存储字段；ADR 0097 固化该合同。

**验证**：MSVC 动态 Debug 与静态 Release 完整构建通过，两套全量测试均为
`115/115`；250 个 clang-tidy 任务以 warnings-as-errors 通过，C++ 格式、48 个 QML
文件的 `qmlformat`/`qmllint` 和 `1765/1765` 翻译门禁通过。真实 Qt Quick 窗口完成
深浅色、常规/紧凑截图与 260px 几何合同，来源 chip 的可访问角色/名称合同通过。
便携 ZIP 与 MSI 已重新生成，SHA-256 分别为
`5a3a40d60f79cfe19c1fe07b5a3fa06de770d0a176cab1f94cb749903768dec5` 和
`e5ba0968f50bae3ff5e7f0abed561f4b87d25b8a600605564c99dce6a6dca97c`；跳过当前主机
不可用的 Windows Installer Service ICE 后，MSI 反编译结构合同与 checksummed release
bundle 组装通过。ICE 本身仍返回已知环境错误 `WIX0217/217`。
...
1. 对照 Wave AI、Warp 当前终端上下文体验及本地 NetCatty 设置布局，确认 AI
   主界面应优先呈现供应商、模型、上下文、历史和工具行为，而不是内部安全架构；
2. 从 AI 设置页移除“隐私诊断”整块，包括端点范围、脱敏计数、运行时计数和
   MCP 边界摘要；底层诊断属性、脱敏、审批和完整 JSONL 调试跟踪保持不变；
3. 模型系统提示改为“拥有该侧栏的唯一终端”，不再引入“另一个终端会话”措辞，
   继续明确禁止发现或选择其它终端；
4. 当前 Debug 真实窗口在 320px 最小侧栏中输入长中文、英文和连续下划线文本，
   均按词或字符换行且没有整体横向滚动条；测试文本已清理；
5. ADR 0096 固化“任务优先、内部诊断不成为用户设置”的产品合同。

**验证**：MSVC 动态 Debug 与静态 Release 构建通过，两套非真实主机测试均为
`115/115`；249 个 clang-tidy 任务以 warnings-as-errors 通过，C++ 格式、48 个 QML
文件的 `qmlformat`/`qmllint` 以及 `1763/1763` 翻译门禁通过。Debug 真实窗口在
320px 最小侧栏完成长中文、英文与连续下划线换行验证。便携 ZIP 与 MSI 均成功生成；
发布总束的 WiX ICE 合同校验仍受当前 Windows Installer Service 不可访问的已知主机
环境问题阻断，错误为 `WIX0217`，不是编译或打包生成失败。
...
1. 发送前的常规预算压缩与供应商 413 后的强制收紧重试，共用结构化压缩摘要；
2. 压缩计数改为“被缩短的上下文项”，同一消息同时移除 provider replay 与截断正文时只计一次，并准确累计移除字符；
3. 当前终端 AI 侧栏在确实缩短旧上下文时显示轻量 `Context optimized` 状态条；仍超预算时显示非阻断警告，不弹窗、不打断输入；
4. 状态条受 260px 窄侧栏几何合同约束，新对话和恢复历史会清除上一轮状态；
5. 该反馈只描述内置供应商请求视图，不改变持久化会话原文，也不引入外部 Agent、跨终端上下文或新的权限步骤。

**验证**：MSVC 动态 Debug 与静态 Release 构建通过，两套完整测试均为 `114/114`；
clang-tidy 以 warnings-as-errors 覆盖 254 个 C++ 翻译单元；48 个 QML 文件、C++ 格式和
1771/1771 翻译目录通过；真实窗口紧凑/常规/深浅色布局门禁通过。便携 ZIP 与 MSI 均重新
生成，SHA-256 分别为 `ada6704c0b5b293695ace0db262913dd97fb655afdb5991a046b0706c602b4a7`
和 `77eb18c1239a2ef32b4bdf765ed6eeb6285503ee4a24a76d86280693939ffa40`。本机 Windows
Installer 服务仍不可访问，因此 WiX ICE 合同检查保持已知的 `WIX0217` 环境限制；产物生成
与 checksummed release bundle 组装成功。
...
1. 新增纯领域 `AiToolEvidence` 判定器，依据完整工具账本区分无工具、已验证、仍等待和不完整；失败优先，未知状态默认仍等待；
2. 判定由持久化 `AiToolActivity` 动态派生，历史恢复后与工具卡保持一致，不维护第二份可漂移状态；
3. system prompt 要求最终回答前逐项核对工具结果，不得把失败、取消、超时、排队或尝试描述为完成；
4. 完成消息若存在失败/取消/仍等待工具，在正文下方显示紧凑、可键盘聚焦的提示；点击直接展开工具时间线，不改写回答、不制造新的审批或全局错误；
5. 修复 `executing` 状态此前未被工具时间线识别为活动中的显示缺口；ADR 0092 固化合同。

**验证**：MSVC 动态 Debug 全量 `118/118` 通过；完整编译数据库 clang-tidy（257 个翻译单元）、C++/QML 格式门禁通过；翻译目录 `1793/1793` 完成；`ai-conversation-model` 覆盖混合工具、失败副作用、完成时仍等待及历史恢复；真实窗口深浅色、紧凑布局与可访问性契约通过。

## 节点 N23 — 剪贴板附件直达输入区（2026-08-15）

**commit**: `1e4e0c6`（feat(ai): paste clipboard attachments into composer）

**范围**：

1. AI 输入框 Ctrl+V 可直接附加 Windows 剪贴板图片和资源管理器复制的图片/UTF-8 文本文件；
2. 普通文本仍走原生粘贴，不改变编辑器行为；图片和文件继续复用异步、有界、当前终端绑定的附件加载链；
3. 真实窗口门禁覆盖图片附加/移除、文件附加/移除与普通文本粘贴，防止输入区回归。

**验证**：`ztermy_ui_layout_runtime_smoke`、`app-controller` 与差异检查通过；工作树提交前干净。
